<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier la source</h1>
                <p class="text-sm text-slate-500 mt-1">Mettre à jour les informations de {{ $source->nom }}.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto">
        <form method="POST" action="{{ route('sources.update', $source) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="nom" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="nom" :value="old('nom', $source->nom)" required autofocus />
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">{{ old('description', $source->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="statut" name="statut" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            <option value="actif" {{ old('statut', $source->statut) === 'actif' ? 'selected' : '' }}>Actif</option>
                            <option value="inactif" {{ old('statut', $source->statut) === 'inactif' ? 'selected' : '' }}>Inactif</option>
                        </select>
