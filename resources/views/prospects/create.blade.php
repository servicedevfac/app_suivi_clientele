<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Créer un prospect</h1>
                <p class="text-sm text-slate-500 mt-1">Ajouter une opportunité commerciale au CRM.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto">
        <form method="POST" action="{{ route('prospects.store') }}" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="nom" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="nom" :value="old('nom')" />
                    ...
                    <div>
                        ...
                        <x-text-input id="telephone" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="telephone" :value="old('telephone')" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="profession" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="profession" :value="old('profession')" />
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{ selectedCampagne: '{{ old('campagne_id', '') }}' }">
                    <!-- Filiale -->
                    <div>
                    ...
                    <div>
                        ...
                        <select id="source_id" name="source_id" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            ...
                            @foreach($sources as $source)
                                <option value="{{ $source->id }}" {{ old('source_id') == $source->id ? 'selected' : '' }}>{{ $source->nom }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="campagne_id" name="campagne_id" x-model="selectedCampagne" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            ...
                            @foreach($campagnes as $campagne)
                                <option value="{{ $campagne->id }}" {{ old('campagne_id') == $campagne->id ? 'selected' : '' }}>{{ $campagne->nom }}</option>
                            @endforeach
                        </select>
                        ...
                        <select id="publication_id" name="publication_id" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            ...
                            @if(isset($publications))
                                @foreach($publications as $pub)
                                    <option value="{{ $pub->id }}" x-show="!selectedCampagne || selectedCampagne == '{{ $pub->campagne_id }}'" {{ old('publication_id') == $pub->id ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        <x-input-label for="statut" value="Statut initial" class="text-slate-700 font-semibold" />
                        <select id="statut" name="statut" required class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            @foreach(['Nouveau', 'Contacté', 'Qualifié', 'En négociation', 'Gagné', 'Perdu'] as $st)
                                <option value="{{ $st }}" {{ old('statut', 'Nouveau') === $st ? 'selected' : '' }}>{{ $st }}</option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="date_contact" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="datetime-local" name="date_contact" :value="old('date_contact')" />
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="besoin" name="besoin" rows="3" placeholder="Quels sont les besoins identifiés pour ce prospect ?" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">{{ old('besoin') }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <x-text-input id="prochain_rappel" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="datetime-local" name="prochain_rappel" :value="old('prochain_rappel')" />
