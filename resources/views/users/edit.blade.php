<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier l'utilisateur</h1>
                <p class="text-sm text-slate-500 mt-1">Mettre à jour les informations de {{ $user->prenom }} {{ $user->nom }}.</p>
            </div>
        </div>
    </x-slot>
        ...
        <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="nom" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="nom" :value="old('nom', $user->nom)" required autofocus autocomplete="family-name" />
                    ...
                    <div>
                        ...
                        <x-text-input id="prenom" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="prenom" :value="old('prenom', $user->prenom)" autocomplete="given-name" />
                    ...
                    <div>
                        ...
                        <x-text-input id="email" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="email" name="email" :value="old('email', $user->email)" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="telephone" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="tel" name="telephone" :value="old('telephone', $user->telephone)" autocomplete="tel" />
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Mot de passe et habilitations de l'utilisateur.</p>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Mot de passe -->
                    <div>
                        <x-input-label for="password" value="Nouveau Mot de passe" class="text-slate-700 font-semibold" />
                        <x-text-input id="password" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="password" name="password" autocomplete="new-password" placeholder="Laisser vide pour ne pas modifier" />
                    ...
                    <div>
                        ...
                        <x-text-input id="password_confirmation" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="password" name="password_confirmation" autocomplete="new-password" />
                    </div>

                    <!-- Rôles -->
                    <div class="md:col-span-2">
                        <x-input-label value="Rôles" class="text-slate-700 font-semibold" />
                        <p class="text-[11px] text-slate-400 mt-0.5">Cochez les rôles affectés à cet utilisateur.</p>
                    ...
                    <div class="md:col-span-2 border-t border-slate-100 pt-6">
                        @if($user->is_active)
                            <label class="relative flex items-start p-4 rounded-xl bg-emerald-50/50 border border-emerald-100 hover:bg-emerald-50 cursor-pointer select-none transition-all duration-150">
                                ...
                                <div class="ml-3 text-xs">
                                    <span class="font-semibold text-emerald-800">Le compte de cet utilisateur est Actif</span>
                                    <span class="block text-[10px] text-emerald-600/80 mt-0.5">Décochez pour désactiver temporairement l'accès de l'utilisateur.</span>
                                </div>
                            </label>
                        @else
                            <label class="relative flex items-start p-4 rounded-xl bg-rose-50/50 border border-rose-100 hover:bg-rose-50 cursor-pointer select-none transition-all duration-150">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" name="is_active" value="1" class="h-4 w-4 rounded border-rose-300 text-rose-600 focus:ring-rose-500">
                                </div>
                                <div class="ml-3 text-xs">
                                    <span class="font-semibold text-rose-800">Le compte de cet utilisateur est Inactif</span>
                                    <span class="block text-[10px] text-rose-600/80 mt-0.5 font-medium">Cochez cette case pour réactiver son accès au système.</span>
                                </div>
                            </label>
                        @endif
                    </div>
