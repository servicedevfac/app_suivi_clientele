<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier la relance</h1>
                <p class="text-sm text-slate-500 mt-1">Mettez à jour les paramètres de planification ou le statut de cette relance.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto">
        <form method="POST" action="{{ route('relances.update', $relance) }}" class="space-y-6">
            @csrf
            @method('PATCH')
                    ...
                    <div>
                        ...
                        <select id="prospect_id" name="prospect_id" required class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            ...
                            @foreach($prospects as $prospect)
                                <option value="{{ $prospect->id }}" {{ old('prospect_id', $relance->prospect_id) == $prospect->id ? 'selected' : '' }}>
                        ...
                        <select id="commercial_id" name="commercial_id" required {{ !auth()->user()->hasRole('Administrateur') ? 'disabled' : '' }} class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm disabled:opacity-50">
                            ...
                            @else
                                ...
                                @foreach($commercials as $comm)
                                    <option value="{{ $comm->id }}" {{ old('commercial_id', $relance->commercial_id) == $comm->id ? 'selected' : '' }}>{{ $comm->name }}</option>
                                @endforeach
                            @endif
                        </select>
                        @if(!auth()->user()->hasRole('Administrateur'))
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="date_relance" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="date" name="date_relance" :value="old('date_relance', $relance->date_relance ? $relance->date_relance->format('Y-m-d') : '')" required />
                        <x-input-error :messages="$errors->get('date_relance')" class="mt-2 text-xs" />
                    </div>

                    <!-- Heure de la relance -->
                    <div>
                        <x-input-label for="heure_relance" value="Heure de la relance" class="text-slate-700 font-semibold" />
                        <x-text-input id="heure_relance" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="time" name="heure_relance" :value="old('heure_relance', $relance->heure_relance ? \Carbon\Carbon::parse($relance->heure_relance)->format('H:i') : '')" />
                    ...
                    <div>
                        ...
                        <select id="canal" name="canal" required class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            <option value="Appel" {{ old('canal', $relance->canal) === 'Appel' ? 'selected' : '' }}>📞 Appel téléphonique</option>
                            <option value="WhatsApp" {{ old('canal', $relance->canal) === 'WhatsApp' ? 'selected' : '' }}>💬 Message WhatsApp</option>
                            <option value="Email" {{ old('canal', $relance->canal) === 'Email' ? 'selected' : '' }}>✉️ Email personnalisé</option>
                            <option value="SMS" {{ old('canal', $relance->canal) === 'SMS' ? 'selected' : '' }}>📱 SMS standard</option>
                            <option value="Rendez-vous" {{ old('canal', $relance->canal) === 'Rendez-vous' ? 'selected' : '' }}>🤝 Rendez-vous physique/visio</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select id="statut" name="statut" required class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">
                            <option value="En attente" {{ old('statut', $relance->statut) === 'En attente' ? 'selected' : '' }}>En attente</option>
                            <option value="Réalisée" {{ old('statut', $relance->statut) === 'Réalisée' ? 'selected' : '' }}>Réalisée</option>
                            <option value="Annulée" {{ old('statut', $relance->statut) === 'Annulée' ? 'selected' : '' }}>Annulée</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="commentaire" name="commentaire" rows="3" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm" placeholder="Ex: Présenter la nouvelle grille... (commentaire optionnel)">{{ old('commentaire', $relance->commentaire) }}</textarea>
