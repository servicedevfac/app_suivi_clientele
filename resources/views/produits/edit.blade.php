<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier le produit</h1>
                <p class="text-sm text-slate-500 mt-1">Mettre à jour les informations de {{ $produit->nom }}.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="nom" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="nom" :value="old('nom', $produit->nom)" required autofocus />
                    ...
                    <div>
                        ...
                        <select id="filiale_id" name="filiale_id" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm" required>
                            ...
                            @foreach($filiales as $filiale)
                                <option value="{{ $filiale->id }}" {{ old('filiale_id', $produit->filiale_id) == $filiale->id ? 'selected' : '' }}>{{ $filiale->nom }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="type" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="text" name="type" :value="old('type', $produit->type)" />
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm">{{ old('description', $produit->description) }}</textarea>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="prix" class="block mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" type="number" step="0.01" name="prix" :value="old('prix', $produit->prix)" />
